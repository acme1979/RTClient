...
namespace WorkStation.FunClass
{
    class CTelnet
    {
        ...
        public string strWorkingData = ""; // 保存从服务器端接收到的数据
        private string strFullLog = "";

        public CTelnet(string Address, int Port, int CommandTimeout)
        {
            address = Address;
            port = Port;
        ...
        #region WriteErrInfo错误日志
        /// <summary>
        /// 错误日志
        /// </summary>
        /// <param name="ErrInfo">错误信息</param>
        private void WriteErrInfo(string ErrInfo)
        {
            string path = Application.StartupPath + "\\ErrorLog.txt";
            if (!File.Exists(path))
            {
                File.WriteAllText(path, ErrInfo);
            }
            else
            {
        ...
        {
            IPHostEntry IPHost = Dns.GetHostEntry(IPAddress.Parse(import));
        ...
        /// <returns>返回0</returns>
        public int WaitForResult(string DataToWaitFor)
        {
            ...
            {
                if (strWorkingData.ToLower().IndexOf(DataToWaitFor.ToLower()) != -1)
                {
                    return 0;
                }
                if (strWorkingData.ToLower().IndexOf("#") != -1)
                {
                    return 0;
                }
